/*:============================================================================
  ...
  @plugindesc | Collision Altering Plugin : Version - 1.0.0 |
...
//-----------------------------------------------------------------------------
  ...
  return this.getMapColliders().concat( this.getCharacterColliders() );
...
//-----------------------------------------------------------------------------
  $.expand( Game_CharacterBase, 'setDestination', function( vec2 )
...
//-----------------------------------------------------------------------------
    ...
    if ( $gameMap.isLoopHorizontal() ) {
      this._x = this._realX = ( this._x ).mod( maxX + 1 )

    } else {
      this._x = this._realX = ( this._x ).clamp( minX, maxX );

    }

    if ( $gameMap.isLoopVertical() ) {
      this._y = this._realY = ( this._y ).mod( maxY + 1 );

    } else {
      this._y = this._realY = ( this._y ).clamp( minY, maxY );
...
//-----------------------------------------------------------------------------
  ...
  let directions = this.getDirectionVectors( false );
...
//-----------------------------------------------------------------------------
$.expand( Game_CharacterBase, 'canTriggerCharacter', function( character )
...
//-----------------------------------------------------------------------------
  ...
  if ( character.isNormalPriority() == false ) {
...
//-----------------------------------------------------------------------------
    ...
    $.alias();
    if ( $gameMap.isPixelMovement() ) this.setupPageHitbox();
...
//-----------------------------------------------------------------------------
    ...
    $.alias();
    if ( $gameMap.isPixelMovement() ) this.resetHitbox();
...
//-----------------------------------------------------------------------------
  ...
  if ( !eventRunning && isValid && isNormal && eventTouch && !jumping ) {
    if ( this.canTriggerCharacter( $gamePlayer ) ) {
...
//-----------------------------------------------------------------------------
    ...
    if ( $gamePlayer.lastTriggeredEvent() == this ) return;
...
//-----------------------------------------------------------------------------
  ...
  if ( this.canStartLocalEvents() && character && character != this ) {
    ...
    if ( character.isTriggerIn( [1, 2] ) ) {
      if ( this.canTriggerCharacter( character ) ) {
        this.clearDestination();
        character.start();
      }
    }
...
//-----------------------------------------------------------------------------
  ...
  return this.getMapColliders( this.isInVehicle() ).concat(
