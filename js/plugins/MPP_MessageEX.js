...
/*:
 * @plugindesc 【ver.3.3】文章表示の機能を拡張したり表示の演出を追加します。
 * @author 木星ペンギン
 * @help [文章の表示]の制御文字:
...
Window_Base.prototype.convertEscapeCharacters = function(text) {
    ...
    text = text.replace(/\x1bSET\[(\d+)\]/gi, () => {
        var index = parseInt(RegExp.$1 || 0);
    ...
    text = text.replace(/\x1b([SIWA])(I?)N\[(\d+)\]/gi, () => {
        return this.mppConvertItemName(RegExp.$1, !!RegExp.$2, parseInt(RegExp.$3));
    });
    return text;
};
...
Window_Message.prototype.convertEscapeCharacters = function(text) {
    ...
    text = text.replace(/\x1bFW\[(\d+)\]/gi, () => {
        this._faceDuration = parseInt(RegExp.$1);
        return '';
    });
    ...
    text = text.replace(/\x1bNC\[(\d+)\]/gi, () => {
        this._nameColorIndex = parseInt(RegExp.$1);
        return '';
    });
...
Window_MessageFace.prototype.loadWindowskin = function() {
    this.windowskin = ImageManager.loadSystem(Params.faceWindow.Windowskin);
};
...
Window_MessageFace.prototype.drawFace = function(faceName, faceIndex) {
    ...
    if (this._moveDuration < 0) {
        fh = Math.min(fh, this._messageWindow.contentsHeight());
        Window_Message.prototype.drawFace.call(this, faceName, faceIndex, 0, 0, fw, fh);
    } else {
        Window_Base.prototype.drawFace.call(this, faceName, faceIndex, 0, 0, fw, fh);
    }
...
Window_MessageName.prototype.loadWindowskin = function() {
    this.windowskin = ImageManager.loadSystem(Params.nameWindow.Windowskin);
};
